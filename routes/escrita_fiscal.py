...
# ---------------------------------------------------------------------------
# Importar XML local — script externo envia o arquivo, app move no Dropbox
# ---------------------------------------------------------------------------
@escrita_fiscal.route('/importar-xml-local', methods=['POST'])
def importar_xml_local():
    ...
    if not current_user.is_authenticated:
        ...
        if not _env_token or req_token != _env_token:
            return jsonify({'erro': 'Não autenticado. Informe X-Import-Token ou faça login.'}), 401

    arquivo = request.files.get('arquivo')
    dropbox_path = request.form.get('dropbox_path', '').strip()
    departamento = dropbox_sync.normalize_departamento(
        request.form.get('departamento', 'Fiscal').strip()
    )

    if not arquivo or not arquivo.filename:
        return jsonify({'erro': 'Parâmetro "arquivo" ausente.'}), 400
    if not dropbox_path:
        return jsonify({'erro': 'Parâmetro "dropbox_path" ausente.'}), 400
    if departamento not in dropbox_sync.DEPARTAMENTOS_CANONICOS:
        return jsonify({'erro': f'Departamento inválido: {departamento}'}), 400
    if not dropbox_sync.is_configured():
        return jsonify({'erro': 'Dropbox não configurado no servidor.'}), 503

    svc = dropbox_sync._service
    nome_arquivo = dropbox_path.rsplit('/', 1)[-1] or arquivo.filename
    ...
    try:
        ...
        return jsonify({'erro': f'Erro ao ler arquivo: {exc}'}), 400

    now = datetime.now()
    _cnpj_cache = _build_cliente_doc_cache()
    _clf = _classify_xml(content)

    # CT-e: não processamos
    if _clf['tipo'] == 'cte':
        return jsonify({
            'resultado': 'ignorado',
            'arquivo': nome_arquivo,
            'motivo': 'CT-e não processado por este endpoint.',
        })

    # Eventos (cancelamento, CCe, manifestação) → ERROS se empresa identificada
    if _clf['tipo'] in ('cancelamento', 'cce', 'manifestacao', 'evento_outro'):
        _proc = _process_evento(_clf, nome_arquivo, content, _cnpj_cache, now)
        if _proc['empresa_nome'] is None:
            return jsonify({
                'resultado': 'ignorado',
                'arquivo': nome_arquivo,
                'motivo': 'Evento sem empresa identificada — arquivo deixado em NOVO.',
            })
        try:
            pasta_ev = svc.pasta_erros(departamento, _proc['empresa_nome'],
                                       _proc['dt'], empresa_numero=_proc['empresa_num'])
            svc.ensure_folder(pasta_ev)
            destino = f"{pasta_ev}/{nome_arquivo}"
            movido = svc.move_file(dropbox_path, destino)
        except DropboxAuthError:
            return jsonify({'erro': 'Falha de autenticação Dropbox.'}), 502
        except Exception as exc:
            return jsonify({'erro': f'Erro ao mover evento para ERROS: {exc}'}), 502
        return jsonify({
            'resultado': 'evento_erros',
            'arquivo': nome_arquivo,
            'empresa': _proc['empresa_nome'],
            'dropbox_destino': destino,
            'movido': movido,
        })
    ...
    try:
        ...
        if len(dest_digits) >= 11:
            found = _find_cliente_by_doc_digits(dest_digits, _cnpj_cache)
            if found:
                _cli = found['id']
        ...
        emit_digits = re.sub(r'\D', '', parsed['header'].get('emit_cnpj', ''))
        if len(emit_digits) >= 11:
            emit_found = _find_cliente_by_doc_digits(emit_digits, _cnpj_cache)
            if emit_found and emit_found['id'] != _cli:
                _emit_cli = emit_found['id']
                if _nome is None:
                    _nome = emit_found['nome_razao_social']
                    _num = emit_found.get('numero_cliente') or None

        if _nome is None:
            return jsonify({
                'resultado': 'ignorado',
                'arquivo': nome_arquivo,
                'motivo': 'Empresa não cadastrada — arquivo deixado em NOVO.',
                'empresa_xml': (parsed['header'].get('dest_nome', '') or '').strip(),
                'cnpj_xml': parsed['header'].get('dest_cnpj', ''),
            })

        result = _save_nfe_dual(parsed, nome_arquivo, 'DROPBOX', content,
                                dest_cli=_cli, emit_cli=_emit_cli,
                                grupo_id=None, vinculos_cache={})

        pasta_imp = svc.pasta_importados(departamento, _nome, _dt, empresa_numero=_num)
        svc.ensure_folder(pasta_imp)
        destino = f"{pasta_imp}/{nome_arquivo}"
        movido = svc.move_file(dropbox_path, destino)

        logger.info('importar_xml_local: %s → %s (%s)', nome_arquivo, destino, result)
        return jsonify({
            'resultado': result,
            'arquivo': nome_arquivo,
            'empresa': _nome,
        ...
        logger.exception('importar_xml_local: erro ao processar %s', nome_arquivo)
        # Tenta mover para ERROS para não deixar o arquivo travado em NOVO
        destino_err = None
        try:
            pasta_err = svc.pasta_erros(
                departamento, _nome or 'DESCONHECIDO', _dt,
                empresa_numero=_num if _nome else None,
            )
            svc.ensure_folder(pasta_err)
            destino_err = f"{pasta_err}/{nome_arquivo}"
            svc.move_file(dropbox_path, destino_err)
        except Exception:
            pass
        return jsonify({
            'resultado': 'erro',
